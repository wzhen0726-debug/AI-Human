...
IN_BLEND = os.environ.get("EYE_IN_BLEND") or os.path.join(ROOT, "01高模修复", "输出", "01_highpoly_repair.blend")
OUT_BLEND = os.environ.get("EYE_OUT_BLEND") or os.path.join(S01A, "输出", "01_1_eye_socket.blend")
SHOT_DIR = os.environ.get("EYE_SHOT_DIR") or os.path.join(WORK, "screenshots")

# 3DDFA反投影结果 (精确定位眼部, 替代暗像素法)
DDFA_JSON = os.environ.get("EYE_DDFA_JSON") or os.path.join(S01A, "3ddfa", "iris_3ddfa.json")
...
EYELID_CONTOUR_JSON = os.environ.get("EYE_CONTOUR_JSON") or os.path.join(S01A, "3ddfa", "eyelid_contour_manual.json")
EYELID_CONTOUR_3DDFA_JSON = os.path.join(S01A, "3ddfa", "eyelid_contour.json")
...
RIM_CONTOUR_HARMONICS = 12     # 保留谐波数(越小越光滑; 12 → 眼角曲率半径≈1mm, 折角14°, 与手描偏差0.33mm)
                               # ⚠ 2026-09-22 实验记录: 收紧到 [4,6] 时 —— L 眼成功(K=4, 最小局部半径2.22mm,
                               #   与手描偏差 2.957→1.226mm, rim环vs手描 max 1.958→1.150mm = 明显更贴手描),
                               #   但 R 眼 rim 带重建连续 6 轮"无法得到单一闭环边界"后放弃、边界死端5个回退,
                               #   rim环顶点残留 328(正常 159~164) → 判定为回归, 已回退到 K=2。
                               #   原因: 两眼平滑偏差几乎相同(L 1.226 / R 1.225mm), 但 R 眼棱柱在更陡处与网格擦出
                               #   碎小孤立环(14/9/9/4点) → 破坏"单一闭环"前提。→ 需按眼做"带重建成功"门控后再收紧。
RIM_CONTOUR_K_FLOOR = 2        # 谐波搜索下限(当前2 = 与历史行为一致)。收紧到4会触发上面记录的 R 眼回归

# ---- 2026-09-22 用户方案A: 按眼【门控】选谐波 K ----
#   背景: 直接收紧到 [4,6] 时 L 眼明显更贴手描(与手描偏差 2.957→1.226mm, rim环vs手描 max 1.958→1.150mm),
#         但 R 眼 rim 带重建连续 6 轮拿不到单一闭环、边界死端回退 → 判定回归(已回退)。
#   做法: 每只眼从 K_HI 往 K_FLOOR 逐档试切，切完立刻重建 rim 带；只有产出单一闭环(+带内无未闭合格)
#         才采用该 K；失败则回滚到该眼操作前的网格快照，换更小的 K。取"该眼能承受的最大 K" = 最贴手描。
RIM_CONTOUR_K_GATE = True      # 总开关(False → 回到 make_eye_socket 内部的自动搜索)
RIM_CONTOUR_K_GATE_HI = 6      # 门控起点(越大越贴手描轮廓, 眼角曲率越小); 该眼全失败则用 K_FLOOR
...
# ---- v88(2026-09-24) 方案1: rim 带外环获取方式 = 【程序化偏移外环 + 环形布尔切割】 ----
#   根因(实测 logs/01a_4.txt): 旧法"沿网格边界顶点邻接 walked 找单一闭环"在边界图有分叉/死端时
#   进入"删坏顶点周围 W×1.6 内的面再重试"(最多 6 轮); 删面本身会造出新边界 → 越删越碎
#   (实测碎环数 2→5→6 个), 6 轮后放弃 → 用断裂外环绕桥接 → 眼角平口切断 + 面疏密混乱;
#   且每轮失败 ≈ 5 分钟, 谐波门控从 K=6 一路回退到 K=2(约 8 次尝试) → 单步 40 分钟。
#   新法: 不再"找环" —— 外环 = 手描轮廓沿【向外法向】程序化偏移 W(等点数重采样);
#         再用环形(垫圈)棱柱对该皮肤带做一次 EXACT 差集 → 带区皮肤被精确切除, 边界=布尔精确闭环;
#         内环 = 手描轮廓同点数 → 直接一一桥接(无最近邻/解绕)。
RIM_BAND_MODE = os.environ.get("EYE_RIM_BAND_MODE") or "ringbool"   # "ringbool"(新) / "walk"(旧法, 保留作回退对比)
RIM_BAND_RING_BOOL_WASHER = False    # True=环形垫圈棱柱(内壁=轮廓+0.03mm内收); False=偏移外环实心棱柱
                                     # (v64 掏空模式下环内本来就是空的, 二者切除结果等价, 实心版少一层"与既有洞边界
                                     #  精确重合"的内壁 → 布尔更稳; 实测对比见报告)
RIM_BAND_RING_BOOL_BACK_EXTRA = 0.30  # 布尔棱柱后伸量 = 该值×眼宽(保证在带区深度方向穿出皮肤; 由实测 y 兜底校验)
RIM_BAND_RING_BOOL_INSET_MM = 0.03    # 垫圈内壁相对轮廓的内收量(仅 washer=True 时生效)
RIM_BAND_RING_BOOL_MAX_FRAG = 10      # 碎环(残留小片)顶点数上限: 超过则判布尔路径失败 → 回退旧路径
RIM_BAND_W_MM = 1.2
RIM_BAND_VERT_CRIT = {"L": False, "R": False}  # 两眼同步(用户要求): 统一保守档, 不再按眼分设          # 沿轮廓向外删多宽的皮肤面
