# -*- coding: utf-8 -*-
"""⚠⚠⚠ 本脚本【不参与自动/半自动流程】, 仅供抢救性恢复使用 (2026-09-16 用户要求) ⚠⚠⚠
原因: 它会把手调点位直接移植进新文件, 导致"一打开就已经完美贴合" —— 假半自动, 用户看不到真实起点。
默认流程(step1 AI打点) 才是真实的半自动: ARP AI 自动摆点 → 用户在GUI里真实调整。
仅在用户明确要求"复用手调成果"时才手动运行本脚本, 且需显式提供 MARKER_SRC 环境变量。
