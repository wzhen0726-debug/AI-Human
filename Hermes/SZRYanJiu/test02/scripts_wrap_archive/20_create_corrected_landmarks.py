...
CORRECTED_BLEND = os.path.join(ROOT, "output", "tripo_tpose_corrected.blend")
OUT_BLEND = os.path.join(ROOT, "output", "landmark_scene_corrected.blend")

print("="*60)
print("创建修正后的特征点空对象场景")
print("="*60)
...
# 定义16个特征点（基于修正后的坐标系）
# 修正后: Z=身高, Y=深度(正面朝-Y), X=宽度(T-pose手臂沿X展开)
landmarks = [
    # 头部
    ...
    ("LM_08_elbow_L", "左肘", (-0.50, -0.05, 1.50)),  # T-pose手臂水平，肘部与肩部同高
    ("LM_09_wrist_L", "左腕", (-0.80, -0.05, 1.50)),  # T-pose手臂水平，腕部与肩部同高
