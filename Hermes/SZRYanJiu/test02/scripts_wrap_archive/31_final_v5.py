...
OUT_BLEND = os.path.join(ROOT, "output", "landmark_scene_v5.blend")
...
# 正确旋转: v4 + 绕Y轴-90°
# 或直接用: 绕X轴-90° + 绕Z轴-90° + 绕Y轴-90°

# 绕X轴-90° (使用matrix_basis)
...
bpy.context.active_object.rotation_euler = (math.radians(45), 0, 0)
...
for name, direction in [('front', (0,-1,0)), ('left', (-1,0,0)), ('top', (0,0,1))]:
    cam.location = bbox['center'] + Vector(direction) * cam_dist
    cam.rotation_euler = (bbox['center'] - cam.location).to_track_quat('-Z', 'Y').to_euler()
    bpy.context.view_layer.update()
    bpy.context.scene.render.filepath = os.path.join(ROOT, "output", "verification", f"v5_{name}.png")
    bpy.ops.render.render(write_still=True)
    print(f"  渲染: v5_{name}.png")
