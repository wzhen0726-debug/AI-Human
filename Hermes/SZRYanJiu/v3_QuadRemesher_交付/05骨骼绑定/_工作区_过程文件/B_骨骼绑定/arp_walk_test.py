"""ARP版(参考骨提取)行走动画测试.
1) 给55骨加mixamorig:前缀(含顶点组) — 与Mixamo行走FBX的data_path匹配
2) 导入行走FBX → 复制action删Hips位移 → slot绑到装数据的channelbag
3) 删参考模型 → depsgraph验证帧1/18变形.
"""
import bpy, os
...
RIG = os.path.join(BASE, "_工作区_过程文件", "B_骨骼绑定", "12_arp_refs_aligned.blend")
OUT = os.path.join(BASE, "_工作区_过程文件", "B_骨骼绑定", "13_arp_walk_test.blend")
...
# ===== 1) 加mixamorig:前缀 (骨骼+顶点组) =====
renamed = 0
for b in rig_arm.data.bones:
...
print(f"加前缀: 骨骼{renamed}根, 顶点组{len(body.vertex_groups)}个")
...
del_names = [o.name for o in bpy.data.objects if o.name != 'MixamoSkeleton'
             and o.name != body.name and 'Eye' not in o.name]
for nm in del_names:
...
print(f"帧18左手: z={lh18.z:.3f} (走路摆臂应≈1.0-1.6, 不应举头顶>1.8)")
print(f"帧18左手位移: {(lh18-lh1).length*100:.1f}cm (应>2cm摆臂)")
print(f"帧18右脚: z={rf18.z:.3f} (应贴地0.0-0.4)")
