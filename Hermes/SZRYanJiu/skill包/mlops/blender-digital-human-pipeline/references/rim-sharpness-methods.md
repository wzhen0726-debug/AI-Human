...
> **补充（2026-09-09）**："QR UseMaterialIds 引导 rim 布线"那条死路有**部分反转**——
> 它不是完全无用，而是 **QR 给的材质边界归属不可靠（xremesh 非确定）**。
> 正确做法是 QR 后几何校正 rim 邻域面的材质归属，让边界贴合 rim。
> 完整方法（含眼窝碗面识别的 tag 拓扑标记法、xremesh 非确定性实测）：
> `references/qr-eye-socket-material-partition.md`
