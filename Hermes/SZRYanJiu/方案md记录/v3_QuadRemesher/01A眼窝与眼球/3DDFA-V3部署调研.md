...
**眼部数据提取**：`face_model.npy` 中 `annotation`/`annotation_tri` 是 8 部件分割的顶点标注，`ldm68/106/134` 是各版本关键点在 BFM 顶点中的索引。眼睛区域的 mask 和关键点索引需从 `face_model.npy` 加载后按文档读取（待 demo 跑通后实测确认具体索引）。
